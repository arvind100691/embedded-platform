...
{
    const auto result = platform::Result<int>::success(42);

    EXPECT_TRUE(result.hasValue());
    EXPECT_TRUE(static_cast<bool>(result));
    EXPECT_EQ(result.error(), platform::ErrorCode::Ok);
...
{
    const auto result = platform::Result<int>::failure(platform::ErrorCode::Timeout);
...
{
    const auto result = platform::Result<void>::success();
...
{
    const auto result = platform::Result<void>::failure(platform::ErrorCode::CommunicationError);
