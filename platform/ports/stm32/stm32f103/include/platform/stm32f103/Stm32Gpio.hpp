...
class Stm32Gpio final : public platform::hal::IGpio, public platform::NonCopyable
{
public:
    ...
    static void handleExti(std::uint16_t pin);

private:
    static int pinToIndex(std::uint16_t pin);

    static std::uint32_t toHalMode(const platform::hal::GpioConfig& config);

    static std::uint32_t toHalPull(platform::hal::GpioPull pull);

    static platform::ErrorCode enableIrq(std::uint16_t pin);

    static platform::ErrorCode disableIrq(std::uint16_t pin);
