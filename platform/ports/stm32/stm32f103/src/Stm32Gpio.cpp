...
platform::Result<void> Stm32Gpio::configure(
...
platform::Result<void> Stm32Gpio::write(
    platform::hal::GpioState state)
{
    HAL_GPIO_WritePin(
        port_,
        pin_,
...
Stm32Gpio::registerInterruptCallback(
    ...
    void* context)
{
    ...
    if (index < 0)
    {
        return platform::Result<void>::failure(
            platform::ErrorCode::InvalidArgument);
    }
...
std::uint32_t Stm32Gpio::toHalMode(
    const platform::hal::GpioConfig& config)
{
    using Direction =
        platform::hal::GpioDirection;

    using Edge =
        platform::hal::GpioInterruptEdge;
...
std::uint32_t Stm32Gpio::toHalPull(
...
platform::ErrorCode Stm32Gpio::enableIrq(
    std::uint16_t pin)
{
    switch (pin)
    {
...
platform::ErrorCode Stm32Gpio::disableIrq(
    std::uint16_t pin)
{
    switch (pin)
    {
...
{
    platform::stm32f103::Stm32Gpio::handleExti(
        GPIO_Pin);
}